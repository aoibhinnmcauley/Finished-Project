...
tShapeOpp::tShapeOpp(Ogre::SceneManager * mSceneMgr, string mName, string materialName)
{
	alive = true;
	time = 0;
	myName = mName;
...
void tShapeOpp::Update(double timeChange, OIS::Keyboard* mKeyboard,vector<vector<vector<Cube*>>>  & array3D, int mainTickValue)
{
...
void tShapeOpp::isDropping(vector<vector<vector<Cube*>>> & array3D)
{
		for(int i = 0; i < cubeAmount; i++ )
		{
...
void tShapeOpp::rotateLeft(double timeChange, OIS::Keyboard* mKeyboard,vector<vector<vector<Cube*>>>  & array3D)
{
...
void tShapeOpp::CheckBounds()
{
			...
			else if(directionFacing == backwards)//Darrens front 
			{	
				cube0boundaryX = 0;
				cube0boundaryX2 = 4;
				cube0boundaryZ = 0;
			...
			else if( directionFacing == forwards)//Darrens back 
			{
			    cube0boundaryX = 0;
				cube0boundaryX2 = 4;
				cube0boundaryZ = 1;
